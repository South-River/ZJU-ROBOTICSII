...
class ICP:
    def __init__(self):
        # max iterations
        self.max_iter = rospy.get_param('/localization/max_iter', 10)
        # distance threshold for filter the matching points
        self.dis_th = rospy.get_param('/localization/dis_th', 0.05)
        # tolerance to stop icp
        self.tolerance = rospy.get_param('/localization/tolerance', 0)
        # min match
        self.min_match = rospy.get_param('/localization/min_match', 2)

        self.corner_num = rospy.get_param('/localization/corner_num', 150)
        self.past_frame_num = rospy.get_param('/localization/past_frame_num', 0)
